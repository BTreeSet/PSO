...
- VPN refresh-token persistence in a user-opaque state directory.
...
- `--totp` or `PSO_PROTON_TOTP` when the account requires TOTP
...
`fetch-logicals` retries transient Proton API failures. If `/vpn/logicals` is temporarily unavailable and `PSO_STATE_DIR/logicals.json` exists, PSO writes that state copy to the requested output path. Use `--no-state-fallback` when a fresh topology response is required.
