...
Proton-Singbox Orchestrator (PSO) is a Rust control plane for producing and maintaining `sing-box` WireGuard endpoints from ProtonVPN account state and declarative routing preferences.
...
- Declarative WireGuard endpoint rendering from `config.template.json` and Proton logical server data.
...
On startup, `run` reads every Proton WireGuard endpoint in the template, refreshes topology, refreshes each account's VPN access token independently from SQLite, registers or refreshes one local WireGuard identity per endpoint, and deploys one combined `sing-box` config. In continuous mode, PSO runs independent endpoint loops for health checks and certificate rotation, plus a topology loop. Endpoint changes are coalesced before rendering, so several account/certificate updates produce one validated config replacement and one SIGHUP.
...
- `state accounts`, `state certs`, `state events`, and `state health` inspect SQLite state.
...
`state accounts` shows known Proton account keys, usernames, and whether a VPN session exists. `state certs` shows endpoint certificate metadata, selected server, endpoint address, assigned tunnel IP, refresh/expiration times, and failure count without printing private keys. `state events` shows runtime events such as session updates and health-triggered certificate refresh outcomes. `state health` shows probe history with raw IP, returned IP, status, and reason.
